...
---
# Hoofdletterpatroonsjabloon gebruiken
...
- **YAML voorwerp:** `exl-id` zou placeholder UUID (b.v., `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`) moeten zijn. De uitgeverspijplijn wijst de echte waarde toe.
- **Adobe productnamen:** gebruik altijd `[!DNL ...]` syntaxis voor de productnamen van Adobe in lichaamstekst en lijsten (b.v., `[!DNL Journey Optimizer]`). Dit is een Experience League-conventie die het vertalen van productnamen verhindert.
- **Bedrijfs objectieve verbindingen:** Gebruik relatieve wegen van het patroondossier aan de bedrijfsdoelstellingen folder: `../../business-objectives/{{category}}/{{filename}}.md`.
- **keybab-case filenames:** Patroonfilename moet kebab-geval zijn dat uit de patroontitel wordt afgeleid. Voorbeeld: &quot;Door gebeurtenis geactiveerd bericht&quot; wordt `event-triggered-messaging.md` .
- **de ketting van de Functie:** Gebruik ` > ` (ruimte, groter-dan, ruimte) als separator tussen stappen.
- **waarden van de Status:** het gebruik van de Begrenzende functies: Vereist, Verondersteld op Plaats, niet van toepassing. Ondersteunende functies: aanbevolen, inbegrepen, niet van toepassing.
- **de fasen van de Implementatie:** Gemeenschappelijke fasennamen omvatten: Opstelling, Configuratie, Activering, Optimalisering, Controle.
- **Vereisten:** gebruik `- [ ]` checkbox syntaxis voor elk punt.
